    ...
    return (
        <div className='w-full'>
            <div className="flex xsm:flex-col sm:flex-row">
                <h1 className="text-[#FF7272] xsm:text-3xl xxl:text-4xl font-bold xsm:text-center lg:max-w-[400px] xl:max-w-fit">{details?.name}</h1>

                <div className='xsm:mt-[20px] sm:mt-0 xsm:mx-auto sm:ml-auto sm:mr-0 flex flex-row'>

                    {(gotUser?.Shelter[0]?.id === details?.id) ? 
                        <button onClick={() => setModalEdit(true)} className="bg-[#6371f1] hover:bg-[#5460d1] dark:bg-[#7F8AF3] dark:hover:bg-[#6a75d3]
                    ...
                    {(gotUser?.Shelter[0]?.id !== details?.id) ?
                        <button onClick={() => setIsOpenDonate(true)} className="bg-white md:h-fit hover:bg-[#c9c8c8] dark:bg-[#1B1A1F] shadow-[0px_0px_25px_-8px_rgba(0,0,0,0.2)]
                    ...
                    {(gotUser?.id !== details?.authorId) ? <div>
                        {(gotUser?.following?.find(shelter => shelter.id === details?.id) || userDetail?.following?.find(shelter => shelter.id === details?.id)) ? (

                            <button onClick={deleteFollow} className="bg-[#FF7272] hover:bg-[#e76464] dark:bg-[#E06161] dark:hover:bg-[#cf5757]
                            rounded-full p-[15px] font-semibold transition-all duration-200 mr-[15px]">
                            ...
                            <button onClick={addFollow} className="bg-[#6371f1] hover:bg-[#5460d1] dark:bg-[#7F8AF3] dark:hover:bg-[#6a75d3]
                            rounded-full p-[15px] font-semibold transition-all duration-200 mr-[15px]">
                    ...
                    <button onClick={() => setIsOpen(true)} className='flex flex-row md:h-fit items-center bg-[#FF7272] hover:bg-[#e76464] transition-all
                    dark:bg-[#E06161] dark:hover:bg-[#cf5757] rounded-full pl-[5px] xl:pr-[25px] xsm:pr-[5px] py-[5px] group duration-300'>
                        ...
                        <span className='text-lg ml-[10px] font-semibold text-white xsm:hidden xl:flex'>Details</span>
                    </button>
                </div>
            </div>
            ...
            <ModalDescription isOpen={isOpen} closeModal={closeModal} details={details} shelterRefetch={shelterRefetch} />
        </div>
    )
}
