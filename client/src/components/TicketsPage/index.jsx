...
function TicketsPage() {
  ...
  const { data: tickets } = useGetTicketsByShelterQuery(
    userDetail.Shelter[0].id
  );
  return (
    <>
      <Toaster />
      <div className="h-screen bg-[#EFF0F3] min-h-screen dark:bg-[#27242C]">
        <div className="flex sm:flex-row xsm:flex-col justify-between">
          <div className="sm:inline">
            <HomeSideBar />
          </div>
          <div
            className="flex flex-col grow h-screen min-w-0 md:pb-[40px] overflow-x-hidden overflow-y-scroll scrollbar-thin 
