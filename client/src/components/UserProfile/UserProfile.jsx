			...
			<div className="mt-8 sm:w-5/6 sm:h-5/6 xsm:w-[90%] mx-auto lg:grid lg:grid-cols-3 lg:grid-rows-1 sm:flex sm:flex-col h-screen xsm:h-auto">
				<div className="mt-16 bg-[#ffffffd7] max-w-md rounded-[3em] mx-auto shadow-[6px_16px_54px_-27px_rgba(133,133,133,0.4)] overflow-hidden min-w-[23vw] dark:bg-[#1B1A1F]">
					<div className="group/item z-0">
						<img
							className="rounded-t-[3em] xl:max-h-[45vh] lg:min-w-[23vw] sm:min-w-[100%] xsm:min-w-[100%] object-right object-cover bg-[#cacaca] z-1 mb-4 lg:h-[35vh]"
							src={details?.profilePic}
							alt="profilePic"
						/>
						{isAuth && userDetail.id === userId && (
							<div className="group/edit invisible opacity-0 cursor-pointer group-hover/item:visible ml-2 z-20 group-hover/item:opacity-100 duration-500 translate-y-[-5.5rem] bg-[#ffffff60] backdrop-blur-sm w-[4rem] h-[4rem] rounded-full absolute align-middle">
								<button
									htmlFor="changeUserImg"
									className="z-20 text-[3rem] text-center text-slate-100 max-w-8 max-h-8 translate-x-2 translate-y-2 cursor-pointer hover:cursor-pointer"
								>
									<RiImageEditFill className="z-30 cursor-pointer hover:cursor-pointer" />
								</button>
								<input
									type="file"
									className="z-40 opacity-0 w-16 h-16 cursor-pointer hover:cursor-pointer translate-y-[-3rem]"
									name="changeUserImg"
						...
						<div className="flex flex-col h-full justify-between">
							{editMode ? (
								...
								>
									<div className="w-fit mx-auto">
										<label className="text-md font-bold text-[#838788] dark:text-[#F0EEEE]">
											Name:{" "}
										</label>
										<input
											type="text"
									...
									<div className="flex flex-row gap-4 justify-center mt-2">
										...
										<button
											onClick={manageCancelButton}
											className="text-white bg-[#FF7272] hover:bg-[#e72d2d] font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 duration-300 dark:bg-[#E06161] dark:hover:bg-[#c92a2a]"
										>
											Cancel
										</button>
									</div>
							...
							<div className="flex flex-row justify-center gap-6 content-end mt-4">
								{details?.following?.length > 0 ? (
									<button
										className="p-2 mb-3 font-bold text-[#838788] hover:text-white drop-shadow-md text-center rounded-md hover:bg-[#83878865] duration-300 dark:text-[#F0EEEE]"
										onClick={manageSheltersFollowedButton}
									>
										<span>{details?.following?.length}</span>
										<span className="ml-2 font-normal">following</span>
									</button>
								) : (
								...
								<span className="p-2 mb-3 font-bold text-[#838788] drop-shadow-md text-center dark:text-[#F0EEEE]">
									<span>Role:</span>
									<span className="ml-2 font-normal">{details?.role}</span>
								</span>
							</div>
						</div>
					</div>
				...
				<div className="lg:col-span-2 sm:mt-8 xsm:mt-8">
					<h5 className="mt-2 mb-2 text-2xl font-bold tracking-tight text-[#838788] drop-shadow-sm text-center dark:text-[#FF7272]">
						Posts
					</h5>
