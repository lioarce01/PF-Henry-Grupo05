...
const Ongs = () => {
	const { data: shelters, isLoading, error, isSuccess, refetch, isFetching } = useGetSheltersQuery("");
	const { data: topShelters, topIsLoading, topError, topIsSuccess, topRefetch } = useTopFiveSheltersQuery("3");
				...
				<div>
					...
					<div className="pb-[130px]">
						<div className="mt-[40px] px-[20px]">
							...
							<h2 className="flex flex-col items-end font-mono tracking-tight font-bold text-[#e07450] mt-[-25px] hover:underline">ALL</h2>
						</div>
							...
							return (
								<div key={s.id} className="flex mt-[20px] ml-[20px]">
									...
									<div className="flex flex-col mt-[15px] w-[190px]">
										...
										<p className="flex flex-col ml-[15px] mt-[-5px] text-[#a59698] text-[0.9rem] font-[700]">{s.city}</p>
									</div>
