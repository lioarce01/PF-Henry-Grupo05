...
'We need your kindness!', 'Take a peak...', 'Do you love animals?']
    ...
    return (
        <div className="grow w-full">
            <div className='flex flex-row'>
                <div className="sm:w-[250px] md:w-[270px] lg:w-[350px] h-[50px] bg-white ml-[20px] mt-[30px] rounded-full">
                    <form onSubmit={e => handleSearch(e)} className='flex flex-row w-full items-center h-full'>
                        <input type="text" className="bg-white h-full pl-[20px] lg:pr-[80px] rounded-full focus:outline-none"
                ...
                {isAuth && <button onClick={() => setOpenCreatePost(true)} className='flex flex-row bg-[#FF7272] ml-auto md:mr-[30px] lg:mr-[30px] xl:mr-[60px]
            ...
            <div className='flex sm:flex-row xsm:flex-col mt-[15px]'>
                    ...
                    {animals.map(anm => {
                        return (
                            <button name={anm.target} onClick={e => handleActive(e)} className={`flex flex-row pl-[5px] lg:py-[5px] lg:pr-[20px] xl:pr-[25px] ml-[10px] transition-all duration-200 xsm:h-[50px] xsm:pr-[5px] 
                                ${! active[anm.target] ? "bg-white hover:bg-[#d6d6d6]" : "bg-[#FF7272] hover:bg-[#e76464]"} rounded-full items-center group`}>
                                <span className={`${! active[anm.target] ? "bg-[#EFF0F3] group-hover:bg-[#e1e2e6]" : "bg-[#d35959] group-hover:bg-[#bb4d4d]"} p-[7px] rounded-full`}>{anm.emoji}</span>
                                <span className={`lg:text-[0.8rem] xl:text-[1rem] lg:ml-[7px] xl:ml-[15px] font-semibold group-hover:text-white xsm:hidden lg:inline ${! active[anm.target] ? "text-[#979b9c]" : "text-white"}`}>{anm.type}</span>
                            </button>
                        )
                    })}
                </div>
                ...
                <button onClick={displayShelters} className='flex flex-row sm:ml-auto xsm:ml-[20px] mt-[20px] hover:underline underline-offset-4 decoration-[#979b9c] sm:mr-[20px] md:mr-[30px] xl:mr-[60px]'>
                    <h2 className="text-[#979b9c] lg:text-[0.9rem] xl:text-[1rem]">Browse Shelters
