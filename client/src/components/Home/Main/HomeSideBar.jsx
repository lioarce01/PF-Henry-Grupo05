...
const SubscriptorsBar = () => {
	...
	return (
		<div className={`flex h-screen bg-white overflow-y-hidden ${expanded ? "md:w-[350px] xsm:w-screen xsm:h-screen xsm:fixed xsm:z-50 sm:static" : "xsm:h-0 sm:h-screen sm:flex w-[140px]"} transition-all duration-300`}>
			<div className={`${expanded ? "md:w-[325px] xsm:w-screen sm:pr-[20px] xsm:pr-0" : "w-[140px]"} h-screen mt-[30px] flex flex-col`}>
				<div className={expanded && "ml-[25px]"}>
					<div className="flex w-full">
						<div className="w-full">
							<Link to="/">
								<div className={`flex group ${expanded ? "flex-row" : "flex-col text-center"}`}>
									<span className={`text-[1.5rem] font-bold text-[#201008] 
										group-hover:text-[#FF7272] transition-all duration-500 font-mono mt-[2px]`}>Paws</span>
									<span className={`text-[1.5rem] font-bold text-[#FF7272] 
						...
						<button onClick={toggleExpanded} className=
							{`xsm:hidden sm:flex text-xl ml-auto bg-[#EFF0F3] absolute items-center rounded-full ${expanded ? "sm:right-[20px] md:right-auto md:left-[320px] p-[15px]" : "left-[125px] p-[5px]"} transition-all duration-300`}>
				...
				<div className={expanded && `xsm:flex sm:flex-row md:flex-col xsm:flex-row xsm:ml-[30px]`}>
					<div className={`xsm:mt-[20px] sm:mt-[45px] ${expanded && "sm:ml-[45px]"}`}>
						<h2 className={`text-[0.9rem] text-[#ACB1B2] font-semibold ${!expanded && "text-center"}`}>Displaying: {!expanded && <br />} {displaying}</h2>
						<ul className="mt-[10px]">
							<li>
								<button onClick={() => handleRender('all')} className={`flex flex-row group ${!expanded && 'mx-auto'}`}>
									<MdPets className={`text-[#bb7070] flex group-hover:text-[#e76f6f] 
									transition-all duration-300 ${expanded ? 'sm:text-2xl xsm:text-xl' : 'text-4xl mx-auto'}`} />
									{expanded && <p className="flex xsm:ml-[10px] sm:ml-[15px] text-[#979b9c] sm:text-[1.1rem] xsm:text-[0.9rem] font-semibold group-hover:underline">All Shelters</p>}
								</button>
							</li>

							<li className="mt-[10px]">
								<button className={`flex flex-row group ${!expanded && 'mx-auto'}`}>
									<TbReportMoney className={`text-[#bb7070] flex group-hover:text-[#e76f6f] 
									transition-all duration-300 ${expanded ? 'sm:text-2xl xsm:text-xl' : 'text-4xl mx-auto'}`} />
									{expanded && <p className="flex xsm:ml-[10px] sm:ml-[15px] text-[#979b9c] sm:text-[1.1rem] xsm:text-[0.9rem] font-semibold group-hover:underline">Subbed Shelters</p>}
								</button>
							</li>

							<li className="mt-[10px]">
								<button onClick={() => handleRender('following')} className={`flex flex-row group ${!expanded && 'mx-auto'}`}>
									<AiFillStar className={`text-2xl text-[#bb7070] flex group-hover:text-[#e76f6f] 
									transition-all duration-300 ${expanded ? 'sm:text-2xl xsm:text-xl' : 'text-4xl mx-auto'}`} />
									{expanded && <p className="flex xsm:ml-[10px] sm:ml-[15px] text-[#979b9c] sm:text-[1.1rem] xsm:text-[0.9rem] font-semibold group-hover:underline">Followed Shelters</p>}
								</button>
							</li>
						</ul>
					</div>
					...
					<div className={`sm:mt-[45px] ${expanded && "md:ml-[45px] sm:ml-auto sm:mr-[140px] xsm:ml-[35px] xsm:mt-[20px]"}`}>
						<h2 className={`text-[0.9rem] text-[#ACB1B2] font-semibold ${!expanded && "text-center"}`}>Specifics</h2>
						<ul className="mt-[5px]">
							<li className="mt-[10px]">
								<button className={`flex flex-row group ${!expanded && 'mx-auto'}`}>
									<FaBook className={`text-[#bb7070] flex mt-[2px] group-hover:text-[#e76f6f] 
									transition-all duration-300 ${expanded ? 'sm:text-2xl xsm:text-xl' : 'text-4xl mx-auto'}`} />
									{expanded && <p className="flex xsm:ml-[10px] sm:ml-[15px] text-[#979b9c] sm:text-[1.1rem] xsm:text-[0.9rem] font-semibold group-hover:underline">Conditions</p>}
								</button>
							</li>
							...
							<li className="mt-[10px]">
								<button className={`group ${!expanded && 'flex mx-auto'}`}>
									...
									<BsPeopleFill className={`text-[#bb7070] flex mt-[2px] group-hover:text-[#e76f6f] 
									transition-all duration-300 ${expanded ? 'sm:text-2xl xsm:text-xl' : 'text-4xl mx-auto'}`} />
									{expanded && <p className="w-[100px] flex xsm:ml-[10px] sm:ml-[15px] text-[#979b9c] sm:text-[1.1rem] xsm:text-[0.9rem] font-semibold group-hover:underline">About Us</p>}
									</Link>
								</button>
							</li>
						</ul>
				...
				<Link to={userDetail?.Shelter?.length > 0 ? `/${userDetail.Shelter[0].id}/profile` : "/createOng"}>
					<div className={` ${expanded ? "sm:ml-[45px] sm:mt-[100px] xsm:mt-[50px] md:mr-[0px] sm:mr-[30px] xsm:mr-[40px] xsm:ml-[35px]" : "mx-[15px] mt-[50px]"} transition-all duration-300`}>
						<div className="xsm:h-[140px] sm:h-[200px] rounded-[20px] bg-gradient-to-bl from-[#FF7272] to-[#be4c4c] hover:to-[#813939]">
				...
				<div className={`md:mt-[50px] flex flex-row ${expanded ? "sm:w-auto sm:mr-[40px] md:w-[250px] ml-[45px] sm:mt-[160px] lg:mt-[100px]" : "ml-[30px] sm:mt-[40px]"}`}>
					{expanded && <h2 className="sm:flex text-[#979b9c] text-[1.1rem] font-semibold xsm:hidden">Night Mode</h2>}

					<div className={`flex ${expanded && "ml-auto sm:mr-[0px] xsm:absolute xsm:top-[40px] xsm:right-[20px] sm:static"}`}>
						<div className="flex flex-row bg-[#F2F1EF] hover:bg-[#e7e6e4] rounded-[30px]">
							<button onClick={toggleDarkMode} className={`flex w-[40px] rounded-[30px] py-[5px] ${! darkMode ? "bg-none" : "bg-[#FF7272]"}`}>
								<HiMoon className={`text-lg mx-auto ${!darkMode ? "text-[#8389A5]" : "text-white"}`} />
							</button>
							<button onClick={toggleDarkMode} className={`flex w-[40px] rounded-[30px] py-[5px] ${darkMode ? "bg-none" : "bg-[#FF7272]"}`}>
								<BsSun className={`text-lg mx-auto ${darkMode ? "text-[#8389A5]" : "text-white"}`} />
							</button>
						</div>
					</div>
				</div>
