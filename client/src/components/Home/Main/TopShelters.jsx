	...
	return (
		<div className="lg:w-[360px] lg:mr-[60px] lg:ml-auto sm:w-full md:mr-[30px] sm:mr-[15px]">
			<div className="flex flex-row px-[20px]">
				<div className="">
					<h2 className="font-bold text-[#979b9c] text-[1.3rem]">Trending Shelters</h2>
				</div>
						...
						{topShelters?.map(shelter => {
							return (
								<Link to={`/${shelter.id}/profile`}>
									<div key={shelter.id} className="ml-[20px] bg-white mt-[15px] px-[15px] py-[15px] mr-[20px] lg:hover:ml-[0px]
										rounded-[30px] group shadow-[0px_15px_20px_-20px_rgba(176,176,176,0.75)] transition-all duration-500">
										<div className="flex flex-row">
											...
											<div className="flex flex-col items-start mt-[10px] ml-[20px]">
												<h1 className="flex flex-col font-semibold text-[#838788]">
													{(shelter.name.length > 18) ? shelter.name.slice(0, 15) + "..." : shelter.name}
												</h1>
												<p className="flex flex-col mt-[-5px] text-[#ACB1B2]">
													{moneyFormat(shelter.budget) + " collected"}
												</p>
											</div>

											<div className="flex flex-row ml-auto mt-[12px]">
												<p className="text-[#4b5bf0] font-bold mr-[2px]">{shelter.followers.length}</p>
												<BsFillPersonFill className="text-[#4b5bf0] mt-[3px]" />
											</div>
										</div>
									</div>
								</Link>
